    ...
    public class BookRepository : IBookRepository
    {
            ...
            using (var connection = _databaseHelper.CreateConnection())
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        var checkBookQuery = "SELECT COUNT(1) FROM Books WHERE BookID = @BookId";
                        var checkCommand = new SqlCommand(checkBookQuery, (SqlConnection)connection, (SqlTransaction)transaction);
                        checkCommand.Parameters.Add(new SqlParameter("@BookId", bookId));
                        int bookCount = (int)checkCommand.ExecuteScalar();

                        // If the book doesn't exist, throw an exception
                        if (bookCount == 0)
                        {
                            throw new ArgumentException($"Book with ID {bookId} not found. Unable to update.");
                        }
...

        {
            ...
            using (var connection = _databaseHelper.CreateConnection())
            {
                ...
                // Append WHERE condition based on SearchBy enum
                switch (searchBy)
                {
                    ...
                    case SearchBy.name:
                        ...
                        break;
                    default:
                        throw new ArgumentOutOfRangeException(nameof(searchBy), "Invalid search criteria.");
                }

                // Group by BookID, BookName, and BookPrice
                query += @"
                            GROUP BY b.BookID, b.BookName, b.BookPrice";

                using (var command = new SqlCommand(query, (SqlConnection)connection))
                {
                    // Handle parameter types based on searchBy
                    switch (searchBy)
                    {
                        case SearchBy.id:
